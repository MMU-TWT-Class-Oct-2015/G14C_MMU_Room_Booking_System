  ...
  <a href="logout.php">Log Out</a></font></p>
        <?php
            ...
            SELECT distinct room_type FROM room WHERE room_type='Lecture room'");
						$id = mysql_query("SELECT distinct member_id FROM member WHERE membership_type='student'");
        ?>
        <center>
        <form name="get_roomid" action="admin_book.php" method="post" >
          <table align="center" width="30%" border="0">
          <p align="center"><font size="7" face="Georgia" color="#000000">
          Member Register</font></p>
          <tr>
          <td><select name='userid'>
							<?php while($row2 = mysql_fetch_assoc($id)): ?>
							<option value='<?php echo $row2["member_id"]?>'><?php echo $row2['member_id']?></option>
							<?php endwhile; ?>
					</select></td>
          </tr>
          <tr>
          ...
          <td>
            ...
            <tr>
            <td>
            <button type="submit" name="submit" action="admin_book.php">NEXT</button>
          </td>
          </tr>
          </table>
        </form>
        ...
        <script>function loadrid(){
            ...
            xmlhttp.open('GET', 'admin_load_roomid.php?room_type='+rtype, true);
